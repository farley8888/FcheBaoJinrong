...
#import "NSString+WebImageAutoSize.h"
#import "WebImageAutoSizeConst.h"

@interface WebImageAutoSizeCache()

@property(nonatomic,strong)NSCache * memCache;
@property(nonnull,strong)NSFileManager *fileManager;

@end
@implementation WebImageAutoSizeCache

+(WebImageAutoSizeCache *)shardCache{
    static WebImageAutoSizeCache *instance = nil;
    static dispatch_once_t oneToken;
    dispatch_once(&oneToken,^{
        instance = [[WebImageAutoSizeCache alloc] init];
    });
    return instance;
}
...
-(void)storeImageSize:(UIImage *)image forKey:(NSString *)key completed:(WebImageAutoSizeCacheCompletionBlock)completedBlock{
...
-(void)storeReloadState:(BOOL)state forKey:(NSString *)key completed:(WebImageAutoSizeCacheCompletionBlock)completedBlock{
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
...
#pragma mark - WebImageAutoSizeCache (private)
...
-(NSData *)dataFromDict:(NSDictionary *)dict{
    ...
    if (error) {
        DMDebugLog(@"ERROR, faild to get json data");
        return nil;
    }
    return data;
}
...
-(NSString *)baseCacheDirectory{
    ...
    NSString *path = [pathOfLibrary stringByAppendingPathComponent:@"WebImageAutoSizeCache"];
    return path;
...
- (void)createBaseDirectoryAtPath:(NSString *)path {
    ...
    if (error) {
        DMDebugLog(@"create cache directory failed, error = %@", error);
    } else {
...
- (void)addDoNotBackupAttribute:(NSString *)path {
    ...
    if (error) {
        DMDebugLog(@"error to set do not backup attribute, error = %@", error);
    }
}
