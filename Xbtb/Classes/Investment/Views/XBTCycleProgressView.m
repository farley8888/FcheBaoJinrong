...
@interface XBTCycleProgressView ()
...
@property (nonatomic, strong) UILabel *countStringLab;
@property (nonatomic, strong) dispatch_source_t timer;
@end

@implementation XBTCycleProgressView
...
- (void)backgroundCyclePath {
    ...
    [self addSubview:self.countStringLab];
    _countStringLab.translatesAutoresizingMaskIntoConstraints = NO;
    [_countStringLab.topAnchor constraintEqualToAnchor:self.countJump.bottomAnchor].active = YES;
    [_countStringLab.centerXAnchor constraintEqualToAnchor:self.centerXAnchor].active = YES;
}
...
- (void)setScareString:(NSString *)scareString
{
    _scareString = scareString;
    _countJump.text = scareString;
}
...
- (UILabel *)countStringLab {
    if (!_countStringLab) {
        _countStringLab = [[UILabel alloc]init];
        _countStringLab.textColor = [UIColor lightGrayColor];
        _countStringLab.font = [UIFont systemFontOfSize:16];
        _countStringLab.textAlignment = NSTextAlignmentCenter;
        _countStringLab.text = @"年化利率";
    }
    return _countStringLab;
}
@end
