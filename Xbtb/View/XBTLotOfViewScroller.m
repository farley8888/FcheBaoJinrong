...
{
    LotOfViewScrollerTopView *topView = [[LotOfViewScrollerTopView alloc]initWithFrame:CGRectMake(0, 0, self.width, 60)];
    WeakSelf
...
- (void)setTableView
{
    for (int i = 0; i < self.titileArray.count; i++) {
        XBTLotOfViewTableView *tabView = [[XBTLotOfViewTableView alloc]initWithFrame:CGRectMake(i*self.width, 0, self.width, self.scrollerView.height)];
        tabView.tag = 1500+i;
        tabView.type = i+1;
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    XBTLotOfViewTableView *tabView = [self viewWithTag:1500 + index];
...
- (void)skipToPageForType:(NSInteger )type
{
    ...
    XBTLotOfViewTableView *tabView = [self viewWithTag:1500 + type];
    if (!tabView.isLoadData) {
        [tabView loadData];
    }
}
