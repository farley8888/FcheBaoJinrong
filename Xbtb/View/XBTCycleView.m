...
@interface XBTCycleView ()<UIScrollViewDelegate>
...
{
    XBTCycleView *cycleView = [[XBTCycleView alloc] initWithFrame:frame images:images currentIndex:currentIndex];
    return cycleView;
}
...
- (instancetype)initWithFrame:(CGRect)frame images:(NSArray *)images currentIndex:(NSInteger)currentIndex
{
    ...
    if (self) {
        _scrollView = [[XBTCycleScrollView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height) andImages:images andCurrentIndex:currentIndex andIndexChange:^(NSInteger index) {
...
{
    XBTCycleView *cycleView = [[XBTCycleView alloc] initWithFrame:frame images:images currentIndex:currentIndex didSelectForIndex:didSelectForIndex];
    return cycleView;
}
