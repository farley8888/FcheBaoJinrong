...
@interface DMGestureLockViewController () <DMGestureLockDelegate, UIAlertViewDelegate>
...
@implementation DMGestureLockViewController
...
//    self.statusLabel = nameLabel;
    ...
    // 九宫格指示器 小图
    DMGestureLockIndicator *gestureLockIndicator = [[DMGestureLockIndicator alloc]initWithFrame:CGRectMake((self.view.frame.size.width - 60) * 0.5, 110, 60, 60)];
    ...
    // 九宫格 手势密码页面
    DMGestureLockView *gestureLockView = [[DMGestureLockView alloc]initWithFrame:CGRectMake(0, self.view.frame.size.height - self.view.frame.size.width - 60 - btnH, self.view.frame.size.width, self.view.frame.size.width)];
...
//            // 保存手势密码
//            [DMGestureLockViewController addGesturesPassword:gesturesPassword];
//        }];
        [UserManager sharedManager].gesString = gesturesPassword;
        [UserManager sharedManager].isOpenGes = YES;
        [DMGestureLockViewController addGesturesPassword:gesturesPassword];
        [NSUserDefaults saveBool:YES key:GesturesPasswordOpenOrClose];
...
- (void)gestureLockView:(DMGestureLockView *)lockView drawRectFinished:(NSMutableString *)gesturePassword {
    ...
    switch (_unlockType) {
        case DMUnlockTypeCreatePsw: // 创建手势密码
        {
            [self createGesturesPassword:gesturePassword];
        }
            break;
        case DMUnlockTypeValidatePsw: // 校验手势密码
        {
